...
!
  ...
  subroutine get_terminal_density_from_file_c(DENSITYFILE, filename_len, CFILE, cfilename_len)&
 bind(C, name="get_terminal_density_from_file_c")

    use iso_c_binding, only: c_ptr
    use utils_c, only: strncpy
    use other_consts, only: MAX_FILENAME_LEN
    use geometry, only: get_terminal_density_from_file
    implicit none

    integer, intent(in) :: filename_len, cfilename_len
    type(c_ptr), value, intent(in) :: DENSITYFILE,CFILE
    character(len=MAX_FILENAME_LEN) :: filename_f,filename_c

    call strncpy(filename_f, DENSITYFILE, filename_len)
    call strncpy(filename_c, CFILE, cfilename_len)
#if defined _WIN32 && defined __INTEL_COMPILER
    call so_get_terminal_density_from_file(filename_f,filename_c)
#else
    call get_terminal_density_from_file(filename_f,filename_c)
#endif
  end subroutine get_terminal_density_from_file_c
!
!###################################################################################
!
...
!###################################################################################
!
  subroutine set_volume_fromrv_c(UNITFILE, filename_len) bind(C, name="set_volume_fromrv_c")

    use iso_c_binding, only: c_ptr
    use utils_c, only: strncpy
    use other_consts, only: MAX_FILENAME_LEN
    use geometry, only: set_volume_fromrv
    implicit none

    !    Parameter List
    integer, intent(in) :: filename_len
    type(c_ptr), value, intent(in) :: UNITFILE
    character(len=MAX_FILENAME_LEN) :: filename_f
    
    call strncpy(filename_f, UNITFILE, filename_len)
#if defined _WIN32 && defined __INTEL_COMPILER
    call so_set_volume_fromrv(filename_f)
#else
    call set_volume_fromrv(filename_f)
#endif
  end subroutine set_volume_fromrv_c
!
!###################################################################################
!
  subroutine set_constriction_from_file_c(CNSTCTFILE, filename_len) bind(C, name="set_constriction_from_file_c")

    use iso_c_binding, only: c_ptr
    use utils_c, only: strncpy
    use other_consts, only: MAX_FILENAME_LEN
    use geometry, only: set_constriction_from_file
    implicit none

    !    Parameter List
    integer, intent(in) :: filename_len
    type(c_ptr), value, intent(in) :: CNSTCTFILE
    character(len=MAX_FILENAME_LEN) :: filename_f

    call strncpy(filename_f, CNSTCTFILE, filename_len)
#if defined _WIN32 && defined __INTEL_COMPILER
    call so_set_constriction_from_file(filename_f)
#else
    call set_constriction_from_file(filename_f)
#endif
  end subroutine set_constriction_from_file_c
!
!###################################################################################
!
