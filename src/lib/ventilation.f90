...
!*evaluate_vent:* Sets up and solves venilation model
  ...
  !DEC$ ATTRIBUTES DLLEXPORT,ALIAS:"SO_EVALUATE_VENT" :: EVALUATE_VENT
    use arrays,only: dp,elem_field,elem_units_below,num_elems,num_units,units,unit_field,unit_density,cc
    use indices,only: ne_Vdot,ne_Vdot0,ne_t_resist,nu_comp,nu_dpdt,nu_pe,nu_vt,nu_vol,nu_Vdot0
    use exports,only: export_1d_elem_field,export_terminal_solution
    use other_consts !PI
    use diagnostics, only: enter_exit
    use geometry,only: set_initial_volume,volume_of_mesh,set_volume_fromrv
    implicit none
    ...
    real(dp) :: ChestWallRestVol,chest_wall_compliance,constrict,COV,&
         ...
         WOBr,WOBe_insp,WOBr_insp,WOB_insp,emphysema_threshold
...
!!! distribute the initial tissue unit volumes along the gravitational axis.
    if(.not.allocated(unit_density))then
      call set_initial_volume(Gdirn,COV,FRC*1.0e+6_dp,RMaxMean,RMinMean)
    else
      !write(*,'('' Predicted FRC = '',F8.3,'' L'')') FRC
      call set_initial_volume(Gdirn,COV,FRC*1.0e+6_dp,RMaxMean,RMinMean)
      !do nunit=1,num_units
          !unit_field(nu_vol,nunit) = undef*unit_field(nu_vol,nunit)

      !end do
      !print*, unit_field(nu_vol,1:5)
      call volume_of_mesh(init_vol,volume_tree)

      !write(*,'('' Calculated FRC for same RV = '',F8.3,'' L'')') init_vol/1.0e+6_dp
    endif
...
!!! calculate the compliance of each tissue unit
    ...
    call tissue_compliance(chest_wall_compliance,undef,emphysema_threshold)
    emphysema_threshold=0.0_dp
    !print*, unit_field(nu_vol,1:5)
    !pause
...
!!! the pressures throughout the tree.
          ...
          call tissue_compliance(chest_wall_compliance,undef,emphysema_threshold) !update the unit compliances, uses 'undef' as input
          totalc = SUM(unit_field(nu_comp,1:num_units)) !the total model compliance
          call update_pleural_pressure(ppl_current) !calculate new pleural pressure
          ...
          time = time + dt
          WRITE(*,'(F7.3,3(F8.1),8(F8.2),3(F8.1))') &
               ...
               elem_field(ne_Vdot0, 679), elem_field(ne_Vdot0, 680),elem_field(ne_Vdot0, 681)
...
!!!###################################################################################
  ...
  subroutine tissue_compliance(chest_wall_compliance,undef,emphysema_threshold)
    use arrays,only: dp,num_units,units,unit_field, unit_density,cc
    ...
    real(dp), intent(in) :: chest_wall_compliance,undef,&
                            emphysema_threshold
    integer :: ne,nunit
    real(dp),parameter :: a = 0.433d0, b = -0.611d0
    !, cc = 2500.0_dp
    real(dp) :: exp_term,lambda,denterm,min_ratio
    real(dp) :: ratio
    ...
    if(.not.allocated(unit_density))then
        cc(1:num_units) = 2500.0_dp
        !ratio(nunit) = unit_field(nu_vol,nunit)/undef
    endif
    !if(.not.allocated(cc)) allocate(cc(num_units))
    !cc(1:num_units) = unit_field(nu_cc,1:num_units)
    !print*,num_units

    !print*,unit_field(nu_cc,1:2)
    do nunit=1,num_units
       if(allocated(unit_density)) then
           if(unit_density(nunit).gt.1.0_dp)then
             ratio = 10.0_dp
	     !ratio = unit_field(nu_vol,nunit)/undef
           elseif(unit_density(nunit).le.emphysema_threshold)then
             ratio = (1.0_dp/unit_density(nunit))+1.15_dp
             if(ratio.gt.50.0_dp)ratio = 50.0_dp
             unit_field(nu_vol,nunit) = ratio*undef
           else
             ratio = unit_field(nu_vol,nunit)/undef
           end if
       else
       ...
       if(lambda.ge.1.15_dp) then
           exp_term=DEXP(0.75d0*(3.0_dp*a+b)*(lambda**2-1.0_dp)**2)
           unit_field(nu_comp,nunit)=cc(nunit)*exp_term/6.0_dp*(3.0_dp*(3.0_dp*a+b)**2 &
           ...
           exp_term=DEXP(0.75d0*(3.0_dp*a+b)*(1.15_dp**2-1.0_dp)**2)
           unit_field(nu_comp,nunit)=cc(nunit)*exp_term/6.0_dp*(3.0_dp*(3.0_dp*a+b)**2 &
                *(1.15_dp**2-1.0_dp)**2/1.15_dp**2+(3.0_dp*a+b) &
                *(1.15_dp**2+1.0_dp)/1.15_dp**4)
           unit_field(nu_comp,nunit)=undef/unit_field(nu_comp,nunit)
           unit_field(nu_comp,nunit)=0.17_dp*cc(nunit)+2.0_dp*(lambda-1)*(unit_field(nu_comp,nunit)-0.17*cc(nunit))
       end if
       ...
       unit_field(nu_pe,nunit) = cc(nunit)/2.0_dp*(3.0_dp*a+b)*(lambda**2.0_dp &
            -1.0_dp)*exp_term/lambda
       !print*, cc(nunit)
    enddo !nunit
    !print*,cc(1:2)
    !print*,unit_field(nu_cc,1:2)
    !print*,unit_field(nu_cc,30308:30309)
...
!!!####################################################################
  ...
  subroutine update_elem_field
    use arrays,only: dp,elem_field,elem_nodes,node_xyz,num_elems,elem_cnstct
    use diagnostics, only: enter_exit
    ...
    do ne=1,num_elems
       ...
       elem_field(ne_length,ne) = DSQRT((node_xyz(1,np2) - &
            ...
            node_xyz(3,np1))**2)
       if(allocated(elem_cnstct))then
         rad = elem_field(ne_radius,ne)*elem_cnstct(ne)
         !elem_field(ne_radius,ne) = rad
       else
       ...
       reynolds=DABS(elem_field(ne_Vdot,ne)*2.0_dp*GAS_DENSITY/ &
            (PI*rad*GAS_VISCOSITY))
       zeta = MAX(1.0_dp,dsqrt(2.0_dp*rad* &
  ...
  subroutine read_params_evaluate_flow (Gdirn, chest_wall_compliance, &
    ...
    do while (ios == 0)
       ...
       if (ios == 0) then
          ...
          case ('emphysema_threshold')
             read(buffer, *, iostat=ios) emphysema_threshold
             print *, 'emphysema_threshold: ', emphysema_threshold
          case default
             print *, 'Skipping invalid label at line', line
          end select
       end if
