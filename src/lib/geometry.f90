...
!This module handles all geometry read/write/generation.
  ...
  public get_terminal_density_from_file
  public set_volume_fromrv
  public set_constriction_from_file
...
!> min and COV values defined.
  ...
  !DEC$ ATTRIBUTES DLLEXPORT,ALIAS:"SO_SET_INITIAL_VOLUME" :: SET_INITIAL_VOLUME
    ...
    real(dp),allocatable :: unit_normden(:)
    character(len=60) :: sub_name
...
!       min_z=MAX(max_z,node_xyz(Gdirn,np2))
!       max_z=MIN(min_z,node_xyz(Gdirn,np2))
    enddo !nunit
...
!###################################################################################
!
  subroutine set_volume_fromrv(UNITFILE)
  !DEC$ ATTRIBUTES DLLEXPORT,ALIAS:"SO_SET_VOLUME_FROMRV" :: SET_VOLUME_FROMRV
    use arrays,only: dp, unit_density,num_units,unit_field,&
         elem_units_below
    use indices,only: nu_vol,nu_vt
    use diagnostics,only: enter_exit
    use other_consts,only: MAX_FILENAME_LEN
	...
	character(len=MAX_FILENAME_LEN),intent(in) :: UNITFILE
    !    Local parameters
    character(len=60) :: sub_name
    character(len=255) :: ctemp1
    real(dp) :: volume_estimate,volume_of_tree,Maxden,Minden,&
        rangeden, Meanden, Rmax, Rmin, Vmax, Vmin, Xi,factor_adjust
    integer :: nunit, ierror

    sub_name = 'set_volume_fromrv'
    call enter_exit(sub_name,1)

    open(10, file = UNITFILE, status = 'old')
    do nunit=1,num_units
      read(unit = 10, fmt='(a)',iostat=ierror) ctemp1
      !print *,  ctemp1
      read(ctemp1,*) unit_field(nu_vol, nunit)
      !unit_field(nu_vol, nunit) = unit_field(nu_vol, nunit)
      unit_field(nu_vt,nunit) = 0.0_dp
      !print *,  unit_field(nu_vol, nunit)
    end do
    close(10)

    
  end subroutine set_volume_fromrv
!
!###################################################################################
!
...
!###################################################################################
!
!*get_terminal_density_from_file:* get density from file
  subroutine get_terminal_density_from_file(DENSITYFILE,CFILE)
    !DEC$ ATTRIBUTES DLLEXPORT,ALIAS:"SO_GET_TERMINAL_DENSITY_FROM_FILE" :: GET_TERMINAL_DENSITY_FROM_FILE
    use arrays,only: unit_density,dp,unit_field,num_units,cc

    use diagnostics,only: enter_exit
    use other_consts,only: MAX_FILENAME_LEN
    implicit none
	
    character(len=MAX_FILENAME_LEN), intent(in) :: DENSITYFILE, CFILE !Input density file
    ! Local Variables
    real(dp) :: terminal_density, nonzero
    real(dp) :: exp_term
    integer :: i, np, ierror, ndnodes,nunit
    character(len=255) :: ctemp1
    character(len=60) :: sub_name
	    
    sub_name = 'get_terminal_density_from_file'
    call enter_exit(sub_name,1)

    open(10, file = DENSITYFILE, status = 'old')
    read_number_of_nodes : do
      read(unit=10,fmt='(a)',iostat=ierror) ctemp1      
      if(index(ctemp1, "Nodes")>0) then		
        call get_final_integer(ctemp1,ndnodes)		
	exit read_number_of_nodes
      endif
    end do read_number_of_nodes
    if(allocated(unit_density)) deallocate (unit_density)
    allocate (unit_density(num_units))
    !np=0
    do nunit=1,num_units
      read(unit=10,fmt='(a)',iostat=ierror) ctemp1
      if(index(ctemp1, 'Density')>0) then
        !np = np+1
	    call get_final_real(ctemp1,terminal_density)
      endif
      unit_density(nunit)=terminal_density
      !if(np.ge.ndnodes) exit
    end do
    nonzero = minval(unit_density, MASK = unit_density.ne.0.0_dp)
    !correct zero denity unit and calculate cc
    do i=1,ndnodes
      if(unit_density(i).eq.0.0_dp)then
        unit_density(i)=nonzero
      !elseif(unit_density(i).gt.0.21_dp)then
        !unit_density(i)=nonzero
      endif
    end do
    if(.not.allocated(cc)) allocate(cc(num_units))
    do nunit=1,num_units
         cc(nunit) = 2500.0_dp
!        if(unit_density(nunit).lt.0.12_dp)then
!        exp_term = DEXP((-1.0_dp) * ((0.08_dp/unit_density(nunit))**3))
!        cc(nunit) = 2500.0_dp * (1.1_dp * exp_term + 0.2_dp)
!      else
!        cc(nunit) = 2500.0_dp * (30.4_dp * ((unit_density(nunit)-0.12_dp)**3) + 1.0_dp)
!      end if
        !cc(nunit) = 2500.0_dp * (20.0_dp*unit_density(nunit)+0.1)
    end do
    close(10)
    !print *,CFILE
    open(11, file=CFILE, status='replace')
    do nunit=1,num_units
        write(11,*) cc(nunit)
    end do
    close(11)
    ...
    !Print *, unit_density
  end subroutine get_terminal_density_from_file
!
!###################################################################################
!
  subroutine set_constriction_from_file(CNSTCTFILE)
      !DEC$ ATTRIBUTES DLLEXPORT,ALIAS:"SO_SET_CONSTRICTION_FROM_FILE" :: SET_CONSTRICTION_FROM_FILE
      use arrays,only:dp,num_elems,elem_field,elem_cnstct
      use indices,only:ne_radius
      use diagnostics,only: enter_exit
      use other_consts,only: MAX_FILENAME_LEN
      implicit none

      character(len=MAX_FILENAME_LEN), intent(in) ::CNSTCTFILE !Input constriction file

      ! Local Variables
      real(dp) :: terminal_density, nonzero
      real(dp) :: exp_term,num
      integer :: i, np, ierror, ndnodes,nelem
      character(len=255) :: ctemp1
      character(len=60) :: sub_name

      sub_name = 'set_constriction_from_file'
      call enter_exit(sub_name,1)
      if(allocated(elem_cnstct)) deallocate (elem_cnstct)
      allocate (elem_cnstct(num_elems))
      open(10, file = CNSTCTFILE, status = 'old')
      do nelem=1,num_elems
        read(unit=10,fmt='(a)',iostat=ierror) ctemp1
        call get_final_real(ctemp1,num)
        elem_cnstct(nelem)=num
      end do
      close(10)
      call enter_exit(sub_name,2)

  end subroutine set_constriction_from_file
!
!###################################################################################
!
